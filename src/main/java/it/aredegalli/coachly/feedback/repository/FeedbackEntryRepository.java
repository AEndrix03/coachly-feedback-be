...
public interface FeedbackEntryRepository extends JpaRepository<FeedbackEntry, UUID>, JpaSpecificationExecutor<FeedbackEntry> {
    ...
    @Query("select avg(f.ratingValue) from FeedbackEntry f where f.type = it.aredegalli.coachly.feedback.model.FeedbackType.REVIEW and f.targetType = :targetType and f.targetId = :targetId and f.deletedAt is null")
    Double averageRating(@Param("targetType") TargetType targetType, @Param("targetId") UUID targetId);

    @Query("select count(f) from FeedbackEntry f where f.type = it.aredegalli.coachly.feedback.model.FeedbackType.REVIEW and f.targetType = :targetType and f.targetId = :targetId and f.deletedAt is null")
    long reviewsCount(@Param("targetType") TargetType targetType, @Param("targetId") UUID targetId);

    @Query("select f.ratingValue, count(f) from FeedbackEntry f where f.type = it.aredegalli.coachly.feedback.model.FeedbackType.REVIEW and f.targetType = :targetType and f.targetId = :targetId and f.deletedAt is null group by f.ratingValue")
